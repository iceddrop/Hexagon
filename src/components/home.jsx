...
export default function Home() {
  ...
  React.useEffect(() => {
    ...
    fetch(
      "https://api.themoviedb.org/3/trending/tv/day?api_key=172f725b29bb276f5c4b6e294a988fc5"
    )
      .then((res) => res.json())
      .then((data) =>
        setTrendingMoviesData(data.results.map((movie) => movie))
      );
  }, []);

  function ion(id) {
    setTrendingMoviesData((movie) =>
      movie.map((each) =>
  ...
  React.useEffect(() => {
    //fetchs trending shows data
    fetch(
      "https://api.themoviedb.org/3/tv/popular?api_key=172f725b29bb276f5c4b6e294a988fc5&language=en-US&page=1"
    )
      .then((res) => res.json())
    ...
    <>
        ...
        <div className="home-overlay">
          <div className="banner">
            ...
            <p className="paragraph-1">
              Your Streaming Shows and Tv Guide. Track, discover and find where
              to watch TV shows from Netflix, Amazon Prime, Hulu, Showtime and
              over 100 more services.
            </p>
      ...
      <section className="trending-movies">
        <div className="carousel">
          <div className="inner-carousel">{trendingEl}</div>
        </div>
      </section>
      <p className="trend-title">Popular Shows</p>
      <section className="trending-movies">
        <div className="carousel">
          <div className="inner-carousel">{popularEl}</div>
        </div>
      </section>
    </>
  );
